...
class IndexedGroup(MutableSequence, ABC):
    ...
    def __setitem__(self, i, item):
        ...
        if not item.location in [e.location for e in self._list]:
            ...
            raise SnirfFormatError(item.location + ' already an element of ' +
...
# generated by sstucker on 2022-07-17
...
class NirsElement(Group):
    ...
    def metaDataTags(self, value):
        if isinstance(value, MetaDataTags):
            self._metaDataTags = copy.copy(value)
            self._metaDataTags._h = self._h[
                'metaDataTags']  # Link the copy to this object's file
        else:
            raise ValueError(
    ...
    def probe(self, value):
        if isinstance(value, Probe):
            self._probe = copy.copy(value)
            self._probe._h = self._h[
                'probe']  # Link the copy to this object's file
        else:
            raise ValueError(
