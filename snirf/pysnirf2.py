...
# generated by sstucker on 2024-12-30
...
class Probe(Group):
    ...
    def __init__(self, var, cfg: SnirfConfig):
        ...
        self._sourcePos2D = _AbsentDataset  # [[<f>,...]]*2
        self._sourcePos3D = _AbsentDataset  # [[<f>,...]]*2
        self._detectorPos2D = _AbsentDataset  # [[<f>,...]]*3
        self._detectorPos3D = _AbsentDataset  # [[<f>,...]]*3
            ...
            if type(nirs.probe) not in [type(None), type(_AbsentGroup)]:
                ...
                for data in nirs.data:
                    if lenSourceLabels is not None and data.measurementLists.sourceIndex is not None and np.max(
                        ...
                        result._add(
                            data.measurementLists.location + '/sourceIndex',
                            'INVALID_SOURCE_INDEX')
                    if lenSources is not None and data.measurementLists.sourceIndex is not None and np.max(
                            data.measurementLists.sourceIndex) > lenSources:
                        result._add(
                            data.measurementLists.location + '/sourceIndex',
                            'INVALID_SOURCE_INDEX')
                    if lenDetectorLabels is not None and data.measurementLists.detectorIndex is not None and np.max(
                        ...
                        result._add(
                            data.measurementLists.location + '/detectorIndex',
                            'INVALID_DETECTOR_INDEX')
